...
export default function CouponFormModal({ open, onClose, coupon }: CouponFormModalProps) {
  const action = coupon ? updateCouponAction : createCouponAction;
  const [state, formAction, pending] = useActionState<CouponFormState | undefined, FormData>(
    action,
    undefined,
  );
  ...
  const heading = coupon ? 'Edit Coupon' : 'New Coupon';
      ...
      >
        ...
        <form action={formAction} className="flex flex-col gap-4">
          ...
          <Field
            ...
            type="number"
            placeholder="e.g. 20"
            defaultValue={coupon ? String(coupon.discountPercent) : undefined}
